...
def team_ranking():
    try:
        if request.method == 'POST':
            ...
            # Subquery to calculate unique days each client was visited
            ...
            # Subquery to calculate total days for clients visited by each team member,
            # counting only unique visit days for each client without overcounting.
            ...
            # Final ranking query with corrected unique client day count calculation
            ranking_query = db.session.query(
                Team_Members.Team_Member_Name,
                days_worked_query.c.days_worked,
                clients_visited_query.c.clients_visited,
                func.sum(team_member_total_days_query.c.total_days_at_clients).label('total_days_at_clients'),
                (days_worked_query.c.days_worked +
                 func.coalesce(clients_visited_query.c.clients_visited, 0) /
                 func.coalesce(func.sum(team_member_total_days_query.c.total_days_at_clients), 1)
                ).label('ranking_score')
             ...
             .group_by(Team_Members.Team_Member_Name, days_worked_query.c.days_worked, clients_visited_query.c.clients_visited) \
