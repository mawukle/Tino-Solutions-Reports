...
def team_ranking():
    try:
        if request.method == 'POST':
            ...
            # CTE for unique days each client was visited within the period
            ...
            # CTE for total unique days each team member spent at clients, treating all Job_IDs for a client on the same day as one
            ...
            # CTE for days worked by each team member, counting unique (Client_Unique_ID, Date) pairs to avoid duplication
            days_worked_query = db.session.query(
                Team_Members.Team_Member_Name,
                func.count(func.distinct(func.concat(Job_Tracking.Client_Unique_ID, Job_Tracking.Date))).label('days_worked')
            ).join(
                job_team_members, job_team_members.Team_Member_ID == Team_Members.Team_Member_ID
            ).join(
                Job_Tracking, Job_Tracking.Job_ID == job_team_members.Job_ID
            ...
            # CTE for clients visited by each team member, counting distinct clients
            ...
            ranking_query = db.session.query(
                ...
                days_worked_query, days_worked_query.c.Team_Member_Name == Team_Members.Team_Member_Name
            ).join(
                clients_visited_query, clients_visited_query.c.Team_Member_Name == Team_Members.Team_Member_Name
            ).outerjoin(
                team_member_total_days_query, team_member_total_days_query.c.Team_Member_Name == Team_Members.Team_Member_Name
            ).order_by(
                desc('ranking_score')
            )
