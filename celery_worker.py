...
# Add this to your existing Celery tasks
@celery.task(bind=True)
def generate_projects_pdf_task(self):
    try:
        # Get your projects data (you'll need to implement this or import your existing function)
        ...
        HTML(
            string=html,
            base_url=request.url_root if request else None  # Handle both web and celery contexts
        ).write_pdf(
            target=pdf_buffer,
        ...
        logging.error(f"PDF generation failed in Celery task: {str(e)}", exc_info=True)
        raise self.retry(exc=e, countdown=60, max_retries=3)
